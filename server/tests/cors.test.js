...
test("only /api/checkout paths are storefront CORS surfaces", () => {
  ...
  assert.equal(isStorefrontApiPath("/api/checkout/leads-digest"), false);
...
test("corsHeadersForRequest never returns * and reflects allowlisted Origin only", () => {
  ...
  const digest = corsHeadersForRequest(
    { headers: { origin: "https://biolabsresearch.co" } },
    "/api/checkout/leads-digest",
    env,
  );
  assert.equal(digest["Access-Control-Allow-Origin"], undefined);
